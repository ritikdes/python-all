...
class Book:
    def __init__(self, title, author, year, date_added):
        self.title = title
        self.author = author
        self.year = year
    ...
    def __str__(self):
        ...
        return f"{self.title} by {self.author} ({self.year}) - {status} | Added on {self.date_added}"
...
class Library:
    ...
    def add_book(self):
        ...
        if title == "" or author == "" or year == "":
            ...
            new_book = Book(title, author, year, date.today().isoformat())
            self.books.append(new_book)
            save_books(filename, self.books)
            print(f"'{title}' added to {self.name} on {date.today()}")
    ...
    try:
        with open(filename, "r") as f:
            for line in f:
                ...
                b = Book(parts[0], parts[1], parts[2], parts[4])
...
def save_books(filename, books):
    with open(filename,"w") as f:
        for book in books:
            ...
            f.write(f"{book.title}|{book.author}|{book.year}|{status}|{book.date_added}\n")
